...
type ScssVariables<Key extends string, Value = string> = {
    [k in Key]?: Value;
} & {
    [k: string]: Value;
};
interface ScssChalk {
    '$colors'?: Partial<Record<'success' | 'warning' | 'danger' | 'error' | 'info', {
        base?: string;
    }>> & ScssVariables<'white' | 'black', string | {
        base?: string;
    }>;
...
type ThemeChalk = ScssChalk & Partial<Record<Themes, ScssChalk>>;
