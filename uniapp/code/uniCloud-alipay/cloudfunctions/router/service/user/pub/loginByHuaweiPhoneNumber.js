module.exports = {
	/**
	 * 用户登录（华为手机号授权登录）
	 * @url user/pub/loginByHuaweiPhoneNumber 前端调用的url参数地址
	 * data 请求参数 说明
	 * @param {String} code 通过 getphonenumber 事件获取到的code
	...
	main: async (event) => {
		...
		// 获取抖音绑定的手机号
		res = await vk.openapi.huawei.decrypt.getPhoneNumber({
			...data,
		...
		if (res.type === "register") {
			...
			if (encryptedKey || openid) {
				// 如果encryptedKey有值，则执行绑定华为
